...
function _getStockItems() {
  // Returns array of { name, whQty, shQty, inStock }
  // Primary source: data.stock  (the inventory list from Stock tab)
  var map = {};

  // 1. Everything in the stock inventory (most important)
  ...
  // 2. Items that have appeared in past challans but aren't in stock list yet
  ...
  return Object.values(map).sort(function(a, b) {
    // Stock items first, then alphabetical
...
function _attachOutItemAutocomplete(nameInp) {
  ...
  function renderDD(q) {
    ...
    var matches = q
      ? items.filter(function(e) { return e.name.toLowerCase().includes(q); })
      : items;  // show ALL stock items when field is blank / on focus

    if (!matches.length) { dd.style.display = 'none'; return; }

    focusIdx = -1;
    dd.innerHTML = matches.slice(0, 60).map(function(e, idx) {
      var src = currentOutSource === 'shop' ? 'shop' : 'warehouse';
      var qty = src === 'shop' ? e.shQty : e.whQty;
      ...
      return '<div class="out-item-opt" data-idx="' + idx + '" data-name="' + e.name.replace(/"/g,'&quot;') + '"'
        + ' onmousedown="event.preventDefault()"'
        + ' onclick="(function(el){'
...
function makeOutItemRow(typePrefill) {
  ...
  rmBtn.className = 'remove-item'; rmBtn.textContent = '✕';
  ...
  // Attach our inventory-aware autocomplete
...
// ─────────────────────────────────────────────────────────────
// Stock deduction — warehouse OR shop
// ─────────────────────────────────────────────────────────────
...
function _commitOut(f) {
  // Create any items that aren't in the stock list yet
  var newItems = _ensureItemsInStock(f.items);
  if (newItems.length) {
    toast('✔ ' + newItems.length + ' new item(s) added to inventory: ' + newItems.join(', '), 3500);
  }
  ...
  var obj = {
    ...
    party:   f.party,
    items:   f.items,
    source:  currentOutSource   // 'warehouse' | 'shop'
  };
...
// ─────────────────────────────────────────────────────────────
// Re-print existing challan from list
// ─────────────────────────────────────────────────────────────
...
function generateOutPdf(o) {
  try {
    ...
    for (var i = 0; i < Math.max(0, 8 - (o.items||[]).length); i++) {
      blanks += '<tr><td style="border:1px solid ' + accent + ';padding:9px;"></td>'
        + '<td style="border:1px solid ' + accent + ';padding:9px;"></td>'
        + '<td style="border:1px solid ' + accent + ';padding:9px;"></td></tr>';
    }
    ...
    var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>' + o.challan + '</title>'
      ...
      + '.pg{padding:24px;max-width:440px;margin:0 auto;}'
      + '.co{font-size:1.5rem;font-weight:900;letter-spacing:3px;color:' + accent + ';text-align:center;}'
      + '.sub{font-size:0.6rem;color:#888;letter-spacing:1px;text-align:center;margin-top:2px;}'
      + '.ct{font-size:0.72rem;font-weight:700;letter-spacing:2px;color:' + accent + ';text-align:center;margin-top:5px;}'
      + '.hr{border-bottom:2px solid ' + accent + ';margin:10px 0 14px;}'
      + 'table{width:100%;border-collapse:collapse;font-size:0.68rem;}'
      + '.mt td{padding:4px 2px;font-size:0.68rem;vertical-align:top;}'
      + '.sig{margin-top:22px;display:flex;justify-content:space-between;font-size:0.6rem;color:#888;}'
      + '.note{margin-top:10px;font-size:0.5rem;color:#aaa;text-align:center;border-top:1px solid #eee;padding-top:6px;}'
      + '.btns{display:flex;gap:10px;justify-content:center;margin:22px 0 10px;}'
      + '.btn{padding:10px 24px;border:none;border-radius:8px;font-size:0.75rem;font-weight:700;cursor:pointer;}'
      + '</style></head><body><div class="pg">'
      + '<div class="co">' + company + '</div>'
      + '<div class="sub">Textile &middot; Surat</div>'
      + '<div class="ct">DELIVERY CHALLAN</div>'
      + '<div class="hr"></div>'
      ...
      + rows + blanks
      + '<tr style="background:' + hdrBg + '">'
      + '<td colspan="2" style="border:1px solid ' + accent + ';padding:6px 8px;font-weight:700;text-align:right">TOTAL</td>'
      + '<td style="border:1px solid ' + accent + ';padding:6px 4px;font-weight:700;text-align:center">' + total + '</td>'
      + '</tr></tbody></table>'
      ...
      + '<button class="btn" style="background:' + accent + ';color:#fff" onclick="window.print()">&#128424; PRINT / PDF</button>'
      + '<button class="btn" style="background:#eee;color:#333" onclick="window.close()">&#10005; Close</button>'
      + '</div></div></body></html>';
  ...
  try {
    ...
    list.innerHTML = rows.slice().reverse().map(function(o) {
      ...
      return '<div class="challan-card">'
        ...
        +   '<div>'
        +     '<div class="ch-num out">' + o.challan + '</div>'
        +     '<div class="ch-date">' + o.date + partyLine + '</div>'
        +   '</div>'
        ...
        +     (linked.length
                ? '<span class="linked-tag">&#128279; ' + linked.length + ' input(s)</span>'
                : '<span style="font-size:0.62rem;color:#bbb">No inputs yet</span>')
        +   '</div>'
        + '</div>'
        + '</div>';
    }).join('');
...
// ─────────────────────────────────────────────────────────────
// Called once when OUT tab first opens (from tab-shared switchTab)
// ─────────────────────────────────────────────────────────────
function initOutTab() {
  setOutSource('warehouse');
